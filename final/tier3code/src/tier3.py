...
dist_precision = 0.2
yaw_precision= math.pi/90 #around 2 degrees
...
def get_laser_scan(msg):
    ...
    safe_dist=0.35 #Safe distance to Obstacles in metres
...
def get_current_posn(pos):
    ...
    #curr_posn_x= pos.pose.pose.position.x
    #curr_posn_y= pos.pose.pose.position.y

    curr_posn_x= pos.pose[1].position.x
    curr_posn_y= pos.pose[1].position.y

    #Define orientation of Robot.
    #rot_q = pos.pose.pose.orientation
    #quaternions = ( rot_q.x , rot_q.y , rot_q.z , rot_q.w )

    quaternions= ( pos.pose[1].orientation.x , pos.pose[1].orientation.y ,
    pos.pose[1].orientation.z ,pos.pose[1].orientation.w )
...
def constant_rotate():
    ...
    global twist_msg, regions, safe_dist, state_description, space_description, err_posn, dist_precision
...
def constant_ahead():
    ...
    global state_description, regions, twist_msg, safe_dist, s, err_posn, dist_precision
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        #rospy.Subscriber("/amcl_pose", PoseWithCovarianceStamped , get_current_posn)
